...
def get_intellectual_pillar(text):
    ...
    return "Specialized Research Applications", "Representing the long-tail of GDELT usage."
...
def generate_data():
    ...
    for w in top_works_raw:
        ...
        impact_lb.append({
            "short_ref": format_short_ref(w), 
            "cites": w.get("cited_by_count", 0),
            "chicago": "<br>".join(textwrap.wrap(format_chicago(w), width=50)), 
            "abstract": "<br>".join(textwrap.wrap(abs_text, width=50)) if abs_text else "No abstract available."
        })
    ...
    return {
        "stats": {
            ...
            "cites": sum(w.get("cited_by_count",0) for w in works)
        },
        "growth": {"academic": dict(sorted(Counter(years).items())), "github": dict(sorted(Counter([int(r.get("created_at")[:4]) for r in repos]).items()))},
        "academic": {"pillars": dict(pillars), "pillar_descs": pillar_descs, "impact_lb": impact_lb, "oa": dict(oa_status), "langs": dict(academic_langs.most_common(10)), "inst": inst_counter.most_common(15), "geo": country_counter.most_common(15)},
...
<head>
    ...
    <style>
        ...
        .score {{ font-weight: 800; color: #f43f5e; }}
    </style>
</head>
<body>
<header>
    <h1>GDELT Global Analysis</h1>
    <div style="text-align: right; font-size: 0.8rem;"><span style="color: var(--accent); font-weight: 800;">Source: OpenAlex API</span><br>English-Only Semantic Audit</div>
</header>
<div class="nav">
...
<div class="content-area">
    <div class="modal-overlay" id="modal-overlay" onclick="closeModal()" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.85); z-index:1000; align-items:center; justify-content:center; padding:1.5rem;"><div class="modal" id="modal-content" onclick="event.stopPropagation()"><span class="close-modal" onclick="closeModal()" style="position:absolute; top:1rem; right:1rem; cursor:pointer; color:#94a3b8; font-size:1.5rem;">&times;</span><h2 id="modal-title" style="color:var(--accent)"></h2><div id="modal-body" style="line-height:1.6"></div></div></div>
    <div id="pillars" class="content active"><div class="grid"><div class="card" id="viz-pillars"><div class="help-icon" onclick="showHelp('pillars')">?</div></div><div class="card" id="viz-oa"></div><div class="card" id="viz-langs"></div></div></div>
    <div id="impact" class="content"><div class="grid"><div class="card full" id="viz-growth"></div><div class="card full" id="viz-cited" style="min-height: 700px;"><div class="help-icon" onclick="showHelp('cited')">?</div></div></div></div>
    <div id="taxonomy" class="content"><div class="grid"><div class="card" id="viz-tax-domain"></div><div class="card" id="viz-tax-field"></div><div class="card" id="viz-tax-subfield"></div><div class="card" id="viz-tax-topic"></div></div></div>
    <div id="software" class="content"><div class="grid"><div class="card" id="viz-maturity"><div class="help-icon" onclick="showHelp('maturity')">?</div></div><div class="card" id="viz-leaderboard"><div class="help-icon" onclick="showHelp('leaderboard')">?</div></div><div class="card" id="viz-gh-fields"></div><div class="card" id="viz-gh-langs"></div></div></div>
</div>
<script>
    const d = {json.dumps(data)};
    const tab20 = {json.dumps(tab20)};
    const layout = {{ paper_bgcolor: 'rgba(0,0,0,0)', plot_bgcolor: 'rgba(0,0,0,0)', font: {{ color: '#f8fafc' }}, margin: {{ t: 50, b: 40, l: 40, r: 40 }}, 
                     xaxis: {{ gridcolor: '#334155', tickformat: ',' }}, yaxis: {{ gridcolor: '#334155', tickformat: ',' }} }};
    ...
    function showHelp(key) {{
        ...
        if(key === 'pillars') {{ title.innerText = "Scholarly Pillar Methodology"; body.innerHTML = "<p>Paradigms derived via direct semantic assessment of abstracts.</p>"; }}
        else if(key === 'leaderboard') {{ title.innerText = "Audit Provenance Logic"; body.innerHTML = "<p>Leaderboard of implementations with explicit source attribution. Numbers are formatted with thousand separators for clarity.</p>"; }}
    ...
    Plotly.newPlot('viz-cited', [{{ 
        x: d.academic.impact_lb.map(x=>x.cites), y: d.academic.impact_lb.map(x=>x.short_ref), type: 'bar', orientation: 'h', marker:{{color:tab20}}, 
        customdata: d.academic.impact_lb.map(x=>[x.chicago, x.abstract, x.cites.toLocaleString()]), 
        hovertemplate: '<b>Citations:</b> %{{customdata[2]}}<br><br><b>Citation:</b><br>%{{customdata[0]}}<br><br><b>Abstract:</b><br>%{{customdata[1]}}<extra></extra>' 
    }}], {{ ...layout, title: 'High-Impact Works', margin:{{l:150, t:80, b:80}}, yaxis:{{autorange:'reversed'}}, xaxis:{{title:'Citations'}}, hoverlabel: {{ align: 'left', bgcolor: '#1e293b', font:{{color:'#f8fafc'}}, maxWidth: 400 }} }}, config);
...
"""
    ...
    print("Master Dashboard with Thousand Separators Everywhere Created.")
